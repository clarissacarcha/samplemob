...
const CategoryList = ({horizontal, rightText = '', filterSearch = 0, homeRefreshing}) => {
  ...
  const renderItem = ({item}) => {
    ...
    return (
      <TouchableOpacity
        ...
        <Text numberOfLines={1} style={[styles.listItemText, {paddingHorizontal: horizontal ? 0 : 10}]}>
          {item.categoryName}
        </Text>
      </TouchableOpacity>
    );
  ...
  const listEmpty = () => (
    <Text style={{color: '#898997', textAlign: 'center', marginVertical: 20}}>No result found</Text>
  );
  ...
  return (
    <View style={[styles.container]}>
      ...
      <View style={styles.listContainer}>{displayComponent()}</View>
    </View>
  );
};
