...
import {Platform, RefreshControl, StyleSheet, View, Image, Text, FlatList, TouchableOpacity} from 'react-native';
...
export const HelpCentre = ({getMyAccount, loading}) => {
          ...
          {walletBalance != null && (
            <View style={[styles.shadow, styles.boxContainer]}>
              ...
              <TouchableOpacity onPress={onPressTopUp}>
                <Text style={{color: '#FCB81A', fontSize: FONT_SIZE.M, paddingHorizontal: 5}}>Top up</Text>
              </TouchableOpacity>
            </View>
          )}
        </>
  ...
  return (
    <FlatList
      data={DATA}
      refreshControl={
        <RefreshControl refreshing={loading} onRefresh={onCashIn} colors={['#FFA700']} tintColor="#FFA700" />
      }
