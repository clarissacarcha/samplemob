...
//SELF IMPORTS
import {Header, MyQrCode, ScanQrOption} from './Components';
...
export const ToktokWalletScanQrHome = ({navigation, route}) => {
  ...
  const scanOptions = [
    ...
    {
      logo: require('toktokwallet/assets/images/qrPayment.png'),
      label: 'QR Payment',
      route: 'ToktokWalletMerchantPayment',
    },
  ];
        ...
        <View style={styles.container}>
          ...
          <ScrollView>
            ...
            <View style={styles.options}>
              {scanOptions.map((item, index) => (
                <ScanQrOption logo={item.logo} label={item.label} route={item.route} />
              ))}
            </View>
          </ScrollView>
          <BuildingBottom />
