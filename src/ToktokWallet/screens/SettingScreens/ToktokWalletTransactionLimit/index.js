...
const TransactionLimit = ({type, incoming, outgoing, index}) => {
  ...
  return (
    <View
      ...
      <View style={{flex: 1}}>
        {incoming && incoming > 0 ? (
          <Text style={{fontFamily: FONT.REGULAR, fontSize: FONT_SIZE.M}}>₱{numberFormat(incoming)}</Text>
        ) : (
          <Text>-</Text>
        )}
      </View>
      <View style={{flex: 1}}>
        {outgoing && outgoing > 0 ? (
          <Text style={{fontFamily: FONT.REGULAR, fontSize: FONT_SIZE.M}}>₱{numberFormat(outgoing)}</Text>
        ) : (
          <Text>-</Text>
        )}
      </View>
  ...
  return (
    <View style={{marginBottom: 20}}>
      <View
        ...
        <Text style={{fontFamily: FONT.SEMI_BOLD, fontSize: FONT_SIZE.M}}>{item.title}</Text>
      </View>
      <View
        style={{
          flexDirection: 'row',
        ...
        <View style={{flex: 1}}>
          {item.walletSize && item.walletSize > 0 ? (
            <Text style={{fontFamily: FONT.REGULAR, fontSize: FONT_SIZE.M}}>₱{numberFormat(item.walletSize)}</Text>
          ) : (
            <Text>-</Text>
          )}
        </View>
