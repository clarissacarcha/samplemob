...
const Circle = ({onPress, value , showPin , error }) => (
    <TouchableHighlight onPress={onPress} underlayColor={COLOR} style={{borderRadius: 10,marginHorizontal: 5, ...(!error ? {} : {borderWidth: 1,borderRadius: 0, borderColor:COLOR.RED}) }}>
      <View style={[styles.inputView, {...(value ? {backgroundColor:COLOR.YELLOW} : {})}]}>
...
const CircleIndicator = ({
    ...
    return (
        <View style={{flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 20, marginBottom: 10,alignSelf:"center"}}>
                {numberBoxes}
        </View>
    )
}
