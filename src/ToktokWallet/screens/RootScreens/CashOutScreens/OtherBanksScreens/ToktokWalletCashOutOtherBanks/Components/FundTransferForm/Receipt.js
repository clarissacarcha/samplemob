  ...
  ({
    ...
    const Proceed = async () => {
      if ((activeAccount && activeAccount >= 0) || savedAccounts.length < 5) {
        if (isSaveAccount && status === 'Successful') {
      ...
      <>
        ...
        <ScrollView style={styles.container} contentContainerStyle={{flexGrow: 1}}>
          <ViewShot
            ...
            <Text style={styles.refNo}>Ref No. {refNo}</Text>

            <Text style={styles.refDate}>{moment(refDate).tz('Asia/Manila').format('MMM D, YYYY hh:mm A')}</Text>
          ...
          <View style={{flex: 1}}>
            <View style={{alignItems: 'center'}}>
              <TouchableOpacity onPress={DownloadReceipt} style={styles.downloadBtn}>
                ...
                <Text style={{fontSize: FONT_SIZE.M, fontFamily: FONT.BOLD, marginLeft: 5, color: '#FF8A48'}}>
                  Download
                </Text>
              </TouchableOpacity>
            </View>
          ...
          {activeAccount == null && savedAccounts.length < 5 && (
            <View style={{paddingHorizontal: 16, flexDirection: 'row', alignItems: 'flex-end'}}>
              ...
              />
              <Text style={{fontFamily: FONT.REGULAR, fontSize: FONT_SIZE.M, marginLeft: 2}}>
