...
const SuccessfulCashOutModal = ({
  ...
  const Proceed = () => {
    // navigation.pop(3)
    if (screenLabel && status !== 'Successful') {
  ...
  const traceNumber = cashoutLogParams?.cashOutUbApiLog?.traceNumber
    ? cashoutLogParams?.cashOutUbApiLog?.traceNumber
    : null;
  const remittanceId = cashoutLogParams?.cashOutUbApiLog?.remittanceId
    ? cashoutLogParams?.cashOutUbApiLog?.remittanceId
    : null;
  const type = cashoutLogParams?.cashOutUbApiLog?.type;
  const cashOutRefNo = cashoutLogParams.referenceNumber
    ? cashoutLogParams.referenceNumber
    : MaskLeftZero(cashoutLogParams.id);

  return (
    <Modal animationType="fade" visible={visible} onRequestClose={Proceed}>
      <Receipt
        refNo={cashOutRefNo}
        ...
        <View style={styles.recipientInfo}>
          ...
          {traceNumber && tokwaAccount.constants.UbFundTransferType == 'api' && (
            <TransactionInfo label="Trace Number" value={traceNumber} />
          )}

          {remittanceId && tokwaAccount.constants.UbFundTransferType == 'api' && (
            <TransactionInfo label="Remittance ID" value={remittanceId} />
          )}

          <TransactionInfo
            label="Amount"
            value={`${tokwaAccount.wallet.currency.code} ${numberFormat(cashoutLogParams.amount)}`}
          />
          {tokwaAccount.constants.UbFundTransferType == 'api' && (
            <>
              <TransactionInfo
                label="Fee"
                value={`${tokwaAccount.wallet.currency.code} ${numberFormat(
                  +cashoutLogParams.providerServiceFee + +cashoutLogParams.systemServiceFee,
                )}`}
              />
              <TransactionInfo
                label="Total Amount"
                value={`${tokwaAccount.wallet.currency.code} ${numberFormat(
                  +cashoutLogParams.providerServiceFee + +cashoutLogParams.systemServiceFee + +cashoutLogParams.amount,
                )}`}
              />
            </>
          )}
