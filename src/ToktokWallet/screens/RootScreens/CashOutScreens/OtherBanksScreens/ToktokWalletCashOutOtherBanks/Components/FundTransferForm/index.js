    ...
    <View style={{flex: 1, marginTop: 16}}>
      ...
      <View style={{padding: 16}}>
        <View style={{}}>
          ...
          {errorListMessage.amount != '' && (
            <Text style={{fontFamily: FONT.REGULAR, fontSize: FONT_SIZE.XS, color: '#F93154'}}>
              {errorListMessage.amount}
            </Text>
          )}
          {amount != '' && bank.id > 0 && !computeLoading && tokwaAccount.constants.UbFundTransferType == 'api' && (
            <Text style={{fontFamily: FONT.REGULAR, fontSize: FONT_SIZE.XS}}>{cfMessage}</Text>
          )}
        </View>
            ...
            style={[
              {
                ...
                borderWidth: 1,
                borderColor: errorListMessage.emailAddress == '' ? 'transparent' : COLOR.RED,
              },
            ]}>
            <TextInput
              style={styles.input}
        ...
        <View style={{marginVertical: 16, marginBottom: 20}}>
          ...
          <TextInput
            ...
            multiline
          />
          <Text style={{fontFamily: FONT.REGULAR, marginTop: 5, fontSize: FONT_SIZE.XS, color: '#929191'}}>
            {note.length}/60
          </Text>
        </View>
      </View>
...
const AccountInfo = ({selectBanks, errorListMessage}) => {
  const {accountName, setAccountName, accountNumber, setAccountNumber, address, setAddress, bank, activeAccount} =
        ...
        <TouchableOpacity
          ...
          style={[
            styles.input,
            {
              ...
              flexDirection: 'row',
              borderWidth: 1,
              borderColor: errorListMessage.bank == '' ? 'transparent' : COLOR.RED,
            },
          ]}>
          <Text style={{fontFamily: FONT.REGULAR, fontSize: FONT_SIZE.M, color: bank.name == '' ? 'gray' : 'black'}}>
            {bank.name == '' ? 'Choose bank here' : bank.name}
          </Text>
          <View style={{flex: 1, alignItems: 'flex-end'}}>
            <VectorIcon iconSet={ICON_SET.Feather} color="black" name="chevron-down" />
          </View>
        </TouchableOpacity>
        {errorListMessage.bank != '' && (
          <Text style={{fontFamily: FONT.REGULAR, fontSize: FONT_SIZE.XS, color: '#F93154'}}>
            {errorListMessage.bank}
          </Text>
        )}
...
export const FundTransferForm = ({selectBanks, screenLabel}) => {
  ...
  const {accountName, accountNumber, bank, note, amount, address, emailAddress, savedAccounts, activeAccount} =
    useContext(ContextCashOut);
  ...
  return (
    <>
      ...
      />
      <AccountInfo selectBanks={selectBanks} errorListMessage={errorListMessage} />
      <Amount
      ...
      <View style={styles.proceedBtn}>
        ...
        {isCertify ? <YellowButton label="Proceed" onPress={onPress} /> : <DisabledButton label="Proceed" />}
      </View>
    </>
  );
};
