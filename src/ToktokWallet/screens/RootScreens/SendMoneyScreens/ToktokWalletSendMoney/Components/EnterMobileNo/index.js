    ...
    if (value.length >= 4) {
      const result = contacts.filter(contact => {
        return (
          ...
          contact.number.toLowerCase().includes(value.toLowerCase())
        );
      });
      return result[0];
    }
        ...
        <View style={styles.content}>
          <TouchableOpacity
            ...
            <View style={{flex: 1, position: 'relative'}}>
              {recipientDetails.id && proceed && (
                <Text style={{fontFamily: FONT.BOLD, fontSize: FONT_SIZE.M, position: 'absolute', marginTop: 5}}>
                  {recipientDetails.person}
                </Text>
              )}
              <TextInput
                ref={inputMobileRef}
                ...
                style={{
                  width: '100%',
                  height: errorMessage ? 35 : 50,
                  padding: 0,
                  fontSize: FONT_SIZE.M,
                  marginTop: recipientDetails.id && proceed ? 5 : 0,
                }}
              ...
              {errorMessage != '' && (
                <Text style={{fontFamily: FONT.REGULAR, fontSize: FONT_SIZE.XS, color: COLOR.RED, marginTop: -5}}>
                  {errorMessage}
                </Text>
              )}
            </View>
          ...
          {recipientDetails.id && errorMessage == '' && (
            <TouchableOpacity
              onPress={() => favoritesRef.current.addFavorites(recipientDetails.id)}
          ...
          <TouchableOpacity
            onPress={() => navigation.navigate('ToktokWalletContacts', {setRecipientInfo: setRecipientMobileNo})}
