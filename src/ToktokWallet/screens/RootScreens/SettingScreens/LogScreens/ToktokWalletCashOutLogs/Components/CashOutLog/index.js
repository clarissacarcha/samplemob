...
export const CashOutLog = ({item, tokwaAccount, index, data}) => {
  ...
  const transactionAmount = `${currencyCode}${numberFormat(
    +item.amount + +item.providerServiceFee + +item.systemServiceFee,
  )}`;
