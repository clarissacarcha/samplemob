        ...
        <>
         { limits?.daily && 
            <Card
                ...
                description={()=><Text style={styles.labelRegular}>Cumulative amount of cash that can be received in your {toktokwalletLabel()}.</Text>}
                limit={+limits?.daily}
                used={used?.daily}
                remaining={(+limits?.daily - +used?.daily)}
                bottomLabel={()=> (
                    <>
                    {!limits?.monthly &&  <Text style={styles.labelRegularSmall}>No limit for Monthly Incoming Transactions</Text>}
                    {!limits?.annual &&  <Text style={styles.labelRegularSmall}>No limit for Annual Incoming Transactions</Text>}
                    </>
                )}
            />
         }
