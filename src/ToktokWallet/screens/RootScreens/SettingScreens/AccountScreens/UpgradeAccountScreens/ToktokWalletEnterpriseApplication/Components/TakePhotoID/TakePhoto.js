  ...
  return (
    <>
      <View style={{flex: 1, padding: 16, marginTop: 0}}>
        <Text style={{fontFamily: FONT.REGULAR, fontSize: FONT_SIZE.M, textAlign: 'center'}}>Front of ID</Text>
        {validID.frontFilename !== '' ? (
          ...
          <ChooseImage
            ...
            placement="front"
            index={index}
            hasError={validID.frontErrorMessage !== ''}
          />
        )}
        {validID.frontErrorMessage !== '' && validID.frontErrorMessage != 'Please make a selection' && (
          <Text style={styles.errorMessage}>{validID.frontErrorMessage}</Text>
        )}
      </View>
      {validID.isBackRequired === 1 && (
        <View style={{flex: 1, padding: 16, marginTop: 0}}>
          <Text style={{fontFamily: FONT.REGULAR, fontSize: FONT_SIZE.M, textAlign: 'center'}}>Back of ID</Text>
          {validID.backFilename !== '' ? (
            ...
            <ChooseImage
              ...
              placement="back"
              index={index}
              hasError={validID.backErrorMessage !== ''}
            />
          )}
          {validID.backErrorMessage !== '' && validID.backErrorMessage !== 'Please make a selection' && (
