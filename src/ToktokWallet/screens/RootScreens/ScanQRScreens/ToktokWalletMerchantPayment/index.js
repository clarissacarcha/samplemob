...
export const ToktokWalletMerchantPayment = ({navigation, route}) => {
        ...
        <View style={styles.container}>
          <RNCamera
            ...
            <View
              ...
              <View>{loading && <ActivityIndicator color={COLOR.YELLOW} size={50} />}</View>

              <View style={styles.TorchView}>
                {image ? <Image source={{uri: image.uri}} style={{height: 100, width: 100}} /> : null}
                <TouchableHighlight onPress={() => setTorch(!torch)} style={styles.torch}>
                  <Image
                    source={
                      torch
                        ? require('toktokwallet/assets/icons/walletScanTorchOn.png')
                        : require('toktokwallet/assets/icons/walletScanTorchOff.png')
                    }
                  />
                </TouchableHighlight>

                <Text style={{color: 'white', fontFamily: FONT.REGULAR, fontSize: FONT_SIZE.L}}>
                  Tap to turn {torch ? 'off' : 'on'}
                </Text>
              </View>
            </View>
            <View style={{marginTop: 25}}>
              <Text style={{color: 'white', fontFamily: FONT.REGULAR, fontSize: FONT_SIZE.L, color: COLOR.YELLOW}}>
