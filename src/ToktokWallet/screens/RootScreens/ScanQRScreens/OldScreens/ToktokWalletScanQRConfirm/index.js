...
//SELF  IMPORTS
import {EnterAmount, EnterNote, ProceedButton, RecipientInfo} from './Components';
...
export const ToktokWalletScanQRConfirm = ({navigation, route}) => {
  ...
  const {recipientInfo, QRInfo} = route.params;
  const session = useSelector(state => state.session);
  const tokwaAccount = useSelector(state => state.toktokWallet);
  const [amount, setAmount] = useState(QRInfo ? QRInfo.amount : '');
            ...
            <View style={styles.walletContent}>
              <View>
                <Text style={{fontSize: 24, fontFamily: FONT.BOLD}}>
                  PHP{' '}
                  {numberFormat(tokwaAccount.wallet.transferableBalance ? tokwaAccount.wallet.transferableBalance : 0)}
                </Text>
