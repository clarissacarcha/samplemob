...
export const CheckIdleState = forwardRef(({children}, ref) => {
  ...
  const durationSettings = useMemo(
    () => (tokwaAccount?.constants?.logoutSessionMinutes ? tokwaAccount.constants.logoutSessionMinutes : 5),
    [tokwaAccount.constants],
  );
  ...
  return (
    <Provider
      ...
      <PromptModal
        visible={showPrompt}
        type="warning"
        message="You’ve been inactive for a while. For security reason, you will be logged out."
