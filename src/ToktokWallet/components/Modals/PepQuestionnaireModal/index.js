...
//SELF IMPORTS
import {SourceOfIncome, SourceOfWealth} from './Components';
...
const Question = ({question, errorMessage, setErrorMessage, index, chooseAnswer, pepInfoAnswer}) => {
  ...
  const onChangeText = text => {
    setErrorMessage(state => {
      state[index] = text.length > 0 && answer.value == 1 ? '' : 'This is a required field.';
      return [...state];
    });
...
export const PepQuestionnaireModal = ({visible, setVisible, onRequestClose, pepInfo, setPepInfo, callback}) => {
  ...
  return (
    <Modal style={styles.container} transparent={false} visible={visible} onRequestClose={closeModal}>
      <ScrollView contentContainerStyle={{flexGrow: 1}} style={styles.headings}>
        <Text style={{fontFamily: FONT.BOLD, fontSize: FONT_SIZE.L, textAlign: 'center'}}>
        ...
        <Text
          ...
          major foreign political party; a senior executive of a foreign government-owned commercial enterprise, and/or
          being a corporation, business or other entity formed by or for the benefit of any such individual; an
        ...
        <View style={{flexDirection: 'row', marginTop: 50, alignItems: 'center'}}>
          <TouchableOpacity
            ...
            style={[styles.answerbox, {height: 20, backgroundColor: agree ? COLOR.YELLOW : 'transparent'}]}
          />
