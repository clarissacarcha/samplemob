      ...
      <View style={{marginRight: 5, width: '90%'}}>
        <Text
          style={{
            ...
            fontFamily: delivery.status === 1 ? FONT.REGULAR : FONT.BOLD,
            color: delivery.status === 1 ? constants.COLOR.DARK : constants.COLOR.BLACK,
          }}>
          {APP_FLAVOR === 'D' && delivery.status === 1 ? getDisplayAddress({stop: stop}) : stop.name}
        </Text>
        {delivery.status > 1 && (
