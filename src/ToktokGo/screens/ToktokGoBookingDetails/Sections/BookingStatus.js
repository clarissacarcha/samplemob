...
const StatusBar = ({label, log, number}) => {
  ...
  if (log) {
    ts =
      moment(log.createdAt, 'MMM DD, YYYY - hh:mm A').format('MMM D, YYYY hh:mm A') != 'Invalid date'
...
export const BookingStatus = ({logs, delivery, session}) => {
  return (
    <View style={[styles.card, {paddingBottom: [2, 3, 4, 5].includes(delivery.status) ? 0 : 0}]}>
