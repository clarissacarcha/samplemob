...
const StatusBar = ({label, log, number}) => {
    ...
    if (log) {
        ts = moment(log.createdAt, "MMM DD, YYYY - hh:mm A").format("MMM D, YYYY hh:mm A") != "Invalid date" 
            ? moment(log.createdAt, "MMM DD, YYYY - hh:mm A").format("MMM D, YYYY hh:mm A") 
            : log.createdAt;
        ...
        <View style={{flex: 1}}>
            ...
            <View style={{flex:1, flexDirection: 'row', marginBottom: 10}}>
                <View style={{marginRight: 25, alignItems: 'center'}}>
                    <FAIcons name={"circle"} size={constants.FONT_SIZE.XS} style={{marginTop:6}} color={log ? constants.COLOR.ORANGE : constants.COLOR.MEDIUM} />
                </View>
                <View style={{flex: 1, flexDirection: 'row', justifyContent: 'space-between'}}>
                    <Text style={{fontFamily: fontFamily, fontSize: constants.FONT_SIZE.M, color: fontColor}}>{label}</Text>
                    <Text style={{fontFamily: constants.FONT_FAMILY.REGULAR, fontSize: constants.FONT_SIZE.M, color: constants.COLOR.FADE,marginTop:2}}>{ts}</Text> 
                </View>
            </View>
...
const Status = ({logs, session }) => {
    ...
    return (
        ...
        <View>
            ...
            <StatusBar number={1} label={"Arrived at Destination"} log={mappedLogs['9']} />
            <StatusBar number={3} label={"Passenger picked up"} log={mappedLogs['5']} />
            <StatusBar number={2} label={"Driver Arrived at pick-up location"} log={mappedLogs['4']} />
            <StatusBar number={4} label={"Driver Accepted"} log={mappedLogs['3']} />
            <View style={{borderBottomWidth: 2, borderColor: constants.COLOR.LIGHT, width: "100%", marginVertical: 15}} />
        </View>
    )
}
    ...
    return (
        <View>
            ...
            <View style={{flex:1, flexDirection: 'row', marginBottom: 5, alignItems: 'center'}}>
                <View style={{marginRight: 25, alignItems: 'center'}}>
                    <FAIcons name={"circle"} size={constants.FONT_SIZE.XS} style={{marginTop:3}} color={constants.COLOR.ORANGE} />
                </View>
                <View style={{flex: 1, flexDirection: 'row', justifyContent: 'space-between'}}>
                    <Text style={{fontFamily: constants.FONT_FAMILY.BOLD, fontSize: constants.FONT_SIZE.L, color: constants.COLOR.ORANGE}}>Cancelled</Text>
                </View>
            </View>
            <View style={{borderBottomWidth: 2, borderColor: constants.COLOR.LIGHT, width: "100%", marginVertical: 15}} />
        </View>
...
export const BookingStatus = ({logs, delivery, session}) => {   

    return (
        <View style={[styles.card, {paddingBottom: [2, 3, 4, 5].includes(delivery.status) ? 100 : 40}]}>
            <View style={styles.directionsBox}>
                ...
                {delivery.status != 7 && <Status logs={logs} session={session} />}
                {delivery.status == 7 && <Cancelled />}
           </View>
        </View>
    )
}
