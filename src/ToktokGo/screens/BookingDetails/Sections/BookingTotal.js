...
import { AccordionBooking } from '../Sections';
    ...
    const getCollectFromCustomer = () => {
        ...
        */
        ...
        if(parseFloat(delivery.collectFromRecipient) != 0) {
            ...
            let deliveryFeeWithDeductions = ((parseFloat(delivery.collectFromRecipient) + parseFloat(delivery.collectFromSender)) - (parseFloat(delivery.partnerBranchOrderFee) + parseFloat(delivery.cashOnDeliveryFee)) - parseFloat(delivery.expressFee)).toFixed(2);
            ...
            // if(delivery.description) { 
                const pabiliFee = `PHP ${(parseFloat(delivery.partnerBranchOrderFee) + parseFloat(delivery.cashOnDeliveryFee)).toFixed(2)}`;
                ...
                {/*-------------------- TOTAL INCOME --------------------*/}
                    <View style={{flex: 1}}>
                        <View>
                            ...
                            {cashTitleText != 0 && <AccordionBooking titleText={"Total"} titleAmount={cashTitleText} subTexts={cashSubTexts} dummyStatus={dummyStatus} />}
                        </View>
                    </View>
                </View>
           </View>
