...
const MainComponent = ({navigation, route}) => {
  const {mobileErrorMessage, mobileNumber, loadVariantId, processGetLoadItems, network} = route.params;

  const prompt = usePrompt();
  const isFocused = useIsFocused();
  const {selectedLoad, setSelectedLoad, search, setSearch, loadFavorite, setLoadFavorite, hasSearch, setHasSearch} =
  ...
  const [getFavoriteLoads, {loading: getFavoritesLoading, error: getFavoritesError}] = useLazyQuery(
    GET_FAVORITE_LOADS,
    {
    ...
    if (value) {
      const filteredSearch = data.filter(item => {
        ...
        return (
          ...
          item.amount.toString().includes(searchKey) ||
  ...
  if (getFavoritesError || postError) {
    return (
      <View style={styles.container}>
        <SomethingWentWrong
          onRefetch={() => processGetFavoriteLoads('refresh')}
          error={getFavoritesError ?? postError}
        />
      </View>
    );
  }
