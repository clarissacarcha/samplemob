...
export const ActivityCard = ({ item, onPress, isLastItem = false }) => {
  ...
  let { amount, convenienceFee, createdAt, destinationNumber, loadDetails, referenceNumber, status, toktokwalletReturnRefNo } = item;
  ...
  return (
    <TouchableOpacity activeOpacity={.7} onPress={onPress} style={[styles.tabContainer, { marginBottom: moderateScale(isLastItem ? 0 : 16) }]} >
      <View style={[ styles.shadow ]}>
        <View style={styles.detailOneContainer}>
          <Text>Service Reference Number  <Text style={{ color: "#FDBA1C", fontFamily: FONT_FAMILY.BOLD }}>{referenceNumber}</Text></Text>
