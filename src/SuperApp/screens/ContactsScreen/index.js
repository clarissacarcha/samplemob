...
//COMPONENTS
import {SearchInput, LoadingIndicator, EmptyList} from 'src/ToktokLoad/components';
...
export const ContactsScreen = ({navigation, route}) => {
  ...
  const setRecipient = item => {
    route.params.onSelectContact({name: item.name, number: onSelectedContact(item)});
  ...
  if (!filteredData) {
    return (
      <View style={styles.center}>
        <ActivityIndicator size={24} color={CONSTANTS.COLOR.YELLOW} />
      </View>
    );
  }
