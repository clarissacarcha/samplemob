  ...
  const [postWalletCashIn, {data, error, loading}] = useMutation(POST_WALLET_CASH_IN, {
    ...
    onCompleted: ({postWalletCashIn}) => {
      navigation.navigate('TokTokWalletCashINPaypandaWebView', {
        ...
        payer_name: `${userstate.person.firstName}${
          userstate.person.middleName ? ' ' + userstate.person.middleName : ''
        } ${userstate.person.lastName}`,
        ...
        <KeyboardAvoidingView
          ...
          <View style={styles.content}>
              ...
              <View style={styles.amountcontent}>
                <View style={{flexDirection: 'row'}}>
                  ...
                  <View style={styles.input}>
                    ...
                    <Text style={{fontFamily: FONT_MEDIUM, fontSize: 30}}>
                      {amount ? numberFormat(amount) : '0.00'}
                    </Text>
                    <FIcon5 name="pen" style={{alignSelf: 'center', marginLeft: 25}} size={18} color="gray" />
                  </View>
                </View>
                <Text style={{color: 'gray', fontSize: SIZES.M, fontFamily: FONT_REGULAR}}>
                  Current Balance PHP {numberFormat(balance)}
                </Text>
                <Text style={{fontFamily: FONT_REGULAR, color: 'red', marginTop: 5, fontSize: 12}}>{message}</Text>
                <Text style={{fontFamily: FONT_REGULAR, color: 'red', marginTop: 5, fontSize: 12}}>
                  {maxLimitMessage}
                </Text>
              </View>
            ) : (
            ...
            <View style={styles.cashinbutton}>
              <TouchableOpacity
                disabled={amount < 1 || amount > transactionType.cashInLimit || disablebtn}
                onPress={confirmAmount}
                style={{
                  height: '100%',
                  width: '100%',
                  backgroundColor: amount < 1 || amount > transactionType.cashInLimit || disablebtn ? 'gray' : DARK,
                  borderRadius: 10,
                ...
                <Text
                  style={{
                    color: amount < 1 || amount > transactionType.cashInLimit || disablebtn ? 'white' : COLOR,
                    fontSize: SIZES.M,
