...
import {GET_BILL_TRANSACTIONS} from 'toktokbills/graphql/model';
...
export const ToktokBillsAllActivities = ({navigation}) => {
  ...
  const [getTransactions, {loading, error}] = useLazyQuery(GET_BILL_TRANSACTIONS, {
  ...
  if (error) {
    return (
      <View style={styles.container}>
        <SomethingWentWrong onRefetch={getTransactions} error={error} />
      </View>
    );
  }
  ...
  return <View style={styles.container}>{AllActivities}</View>;
};
