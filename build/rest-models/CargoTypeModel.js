...
CargoTypeModel.create = (req) => __awaiter(void 0, void 0, void 0, function* () {
    const date = dateFormat(new Date(), "yyyy-mm-dd HH:MM:ss");
    let query = MysqlUtility_1.MysqlUtility.mergeLines([
        "insert into tok_cargo_types",
...
CargoTypeModel.read = (id) => __awaiter(void 0, void 0, void 0, function* () {
    let query = "select * from tok_cargo_types where id = ?";
    return yield pool.query(query, [id]);
});
CargoTypeModel.update = (req) => __awaiter(void 0, void 0, void 0, function* () {
    const date = dateFormat(new Date(), "yyyy-mm-dd HH:MM:ss");
    let query = MysqlUtility_1.MysqlUtility.mergeLines([
        "update tok_cargo_types set",
        "type = ? ,",
        "updated_at = ? ",
        "where id = ?",
    ]);
...
CargoTypeModel.delete = (id) => __awaiter(void 0, void 0, void 0, function* () {
    const date = dateFormat(new Date(), "yyyy-mm-dd HH:MM:ss");
    let query = "update tok_cargo_types set status = 0, updated_at = ? where id = ?";
    return yield pool.query(query, [date, id]);
});
CargoTypeModel.list = (req) => __awaiter(void 0, void 0, void 0, function* () {
    let query = "";
    let values = [];
    let counterQuery = "";
    ...
    if (req.searchstring != "") {
        ...
        counterQuery = MysqlUtility_1.MysqlUtility.mergeLines([
            "select count(id) as totalRows from tok_cargo_types",
            "where type like ? ",
            "and status = ?",
        ]);
        values = [
