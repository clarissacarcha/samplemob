...
export class DriverModel {
  static create = async (req: any) => {
    ...
    let values = [req.license_number, 0, 2, 1, date, date, req.userId, null];
  ...
  static getGeneralInfo = async (token: string) => {
    ...
    const usernameData = await pool.query(queryUsername, [
      userId[0].tok_users_id,
    ]);
  ...
  static getDeliveryHistory = async (req: any) => {
    ...
    let queryDeliveries =
      "select id, description, created_at as date from tok_deliveries where tok_driver_id = ?";
    ...
    let queryDeliveryLogs =
      "select tok_delivery_id from tok_delivery_logs where tok_delivery_id in (" +
      driverDeliveriesIds +
      ")";
    ...
    const orderColumns = ["id", "description", "created_at"];
    ...
    if (req.searchstring != "") {
      query = MysqlUtility.bindValues(
        MysqlUtility.mergeLines([
          "select id, description, DATE_FORMAT(created_at,'%d/%m/%Y') as date from tok_deliveries where id in (" +
            driverDeliveriesIds +
            ")",
      ...
      counterQuery = MysqlUtility.mergeLines([
        "select count(id) as totalRows from tok_deliveries where id in (" +
          driverDeliveriesIds +
          ")",
        "and status = ?",
      ]);
