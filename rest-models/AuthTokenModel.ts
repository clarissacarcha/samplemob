...
export class AuthTokenModel {
  static create = async (
    ...
    let values = [token, userId, roles, permissions, date, date];
  ...
  static verifyLogin = async (token: string) => {
    ...
    let query =
      "select * from tok_auth_tokens where token = ? and last_activity > ?";

    const result = await pool.query(query, [token, mysqlDateFormat]);

    if (result.length > 0) {
      let query =
        "update tok_auth_tokens set last_activity = ? where token = ?";
